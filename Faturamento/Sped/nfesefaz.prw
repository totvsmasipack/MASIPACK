...
Local aMsPedC		:= {}		//- ESPECIFICO GRUPO MASIPACK
Local _nY			:= 0		//- ESPECIFICO GRUPO MASIPACK
...
If cTipo == "1"
	...
	IF cEmpAnt $ '01|10|15'
		If !(SF2->F2_MSIMPR == 'S')
			cNFe := ""
			cString := ""
			FwAlertError("Atencao, nota nao podera ser transmitida -  Espelho nao foi impresso: " + cNota)
...
/*/	QUANDO MUDAR MSG PARA NF, TIRAR ESSE COMENTADO, POIS ESSE BLOCO DO PADRAO
...
/*/
						...
						If Ascan(aRecPed, { |x| x == SC5->(RECNO()) }) == 0
							...
							IF SF2->F2_COND = 'VIN'
								cRetForm := ' | VENDA INTERNA/FUNCIONARIO '
							...
							IF cEmpAnt $ "15" 
								...
								//-- Tratamento para a integracaoo entre WMS Logix X ERP Protheus 
								If SC5->( FieldPos("C5_ORIGEM") ) > 0 .And. 'LOGIX' $ Upper(SC5->C5_ORIGEM)
									LgxMsgNfs()
								EndIf
								...
								If !EMPTY(SC5->C5_PC)
									...
									cRetForm := " | Nº Ped. Cliente: " + ALLTRIM(SC5->C5_PC)
									If !(Alltrim(cRetForm) $ cMensCli)
										cMensCli += cRetForm
									Endif
									cMensCli += " | *** N/NF REF PV: " + SC5->C5_NUM + " ***"
							...
							If !(cEmpAnt $ "40|45")
								...
								If SZO->(FOUND()) .And. !Empty(Alltrim(MEMOLINE(SZO->ZO_MENS,130,1))) 
									...
									Next _nY
									If !( Alltrim(cRetForm) $ Alltrim(cMensCli) )
										cMensCli += cRetForm
									Endif
								EndIf
							Endif
							
							If !(cEmpAnt $ "40|45") .And. !Empty(SC5->C5_MENPAD)
								
								cRetForm := FORMULA(SC5->C5_MENPAD)
								if !(ValType(cRetForm) == nil) .and. !(AllTrim(cRetForm) $ cMensFis)
									If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
										cMensFis += " "
									EndIf
									cMensFis += AllTrim(cRetForm)
							...
							ElseIf cEmpAnt $ "40|45" .And. !Empty(SC5->C5_MENPAD)
								
								DbSelectArea("SM4")
								DbSetOrder(1)
								IF SM4->(DbSeek(xFilial("SM4")+SC5->C5_MENPAD))
									cRetForm := SM4->M4_FORMULA
									IF !EMPTY(SM4->M4_MSTXT1)
										If Len(cRetForm) > 0 .And. SubStr(cRetForm, Len(cRetForm), 1) <> " "
											cRetForm += " "
										EndIf
										cRetForm += SM4->M4_MSTXT1
									ENDIF	 
									cMensFis += AllTrim(cRetForm)                                            
								ENDIF
						...
						*/
						...
						If cEmpAnt  $ "01|10"
							SZB->(DbSetorder(1)) 
							If SZB->(DbSeek(xFilial("SZB")+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV))
								_cDescPro := TRIM(SZB->ZB_DESC01)+" - Local: "+SB1->B1_LOCAL 
							EndIf
						EndIf
						
						If Empty(_cDescPro)   
							SB5->(dbSetOrder(1))
							IF SB5->(DbSeek(xFilial("SB5")+(cAliasSD2)->D2_COD+(cAliasSD2)))
								IF !EMPTY(SB5->B5_CEME)
									_cDescPro := TRIM(SB5->B5_CEME)+IIF(cEmpAnt $ "01|10"," - Local: "+SB1->B1_LOCAL,"")
								ENDIF 
							ELSE
								_cDescPro 	:= TRIM(SB1->B1_DESC)+IIF(cEmpAnt $ "01|10"," - Local: "+SB1->B1_LOCAL,"")
							ENDIF
						EndIf   
						
						_cNCM := SB1->B1_POSIPI
						
						IF SUBSTR(SC5->C5_NATUREZ,1,5) == "V07.1"         
							_cNCM := "84229090"
						ENDIF 
						//-----------------------------------
						//- FINAL DA CUSTOMIZACAO			-
						...
						*/
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							_cDescPro,; //	cDescProd,;  -> EXCLUSIVO GRUPO MASIPACK
						...
						If !Empty(SC6->C6_PEDCLI)
							aadd(aMsPedC,{AllTrim(SC6->C6_PEDCLI),AllTrim(SC6->C6_MSITPEC)}) 
						ELSE	
							aadd(aMsPedC,{}) 
						EndIf
						//-----------------------------------
			...
			While ! (cAliasSD1)->(EoF()) .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				*/
				...
				IF SB5->(FOUND()) .And. !EMPTY(SB5->B5_CEME)
					_cDescPro := TRIM(SB5->B5_CEME)+IIF(cEmpAnt $ "01|10"," - Local: "+SB1->B1_LOCAL,"")
				ELSE
					_cDescPro := TRIM(SB1->B1_DESC)+IIF(cEmpAnt $ "01_10"," - Local: "+SB1->B1_LOCAL,"")
				ENDIF 
	   	        	            
				//- TROCA A NCM QDO IMPORTACAO
	            _cNCM := SB1->B1_POSIPI
	            IF SUBSTR(SC5->C5_NATUREZ,1,5) == "V07.1"
		            _cNCM := "84229090"
	            ENDIF
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
							...
							@aTotICMSST,len(aProd), nX, @nValDifer, cIndPres, lExpCDL, @aMonof02, @aMonof15, @lMonof53, @lMonof61, aICMSMono[nX], aBenef[nX],lDeduzDeson)
	Next nX
...
If Len(aPedido)>0
	If !Empty(aPedido[01]) .or. !Empty(aPedido[02]) .or. !Empty(aPedido[03])
		...
		//-----------------------------------
			//- ESPECIFICO GRUPO MASIPACK		-
			//-----------------------------------
			If AllTrim(SA1->A1_COD) $ SuperGetMv("MS_NFE0001",," ")
				cString += NfeTag('<xPed>',aPedido[02])
			EndIf
			//-----------------------------------
			//- FINAL DA CUSTOMIZACAO			-
			//-----------------------------------
