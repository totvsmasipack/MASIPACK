...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			//-------------------------------------------------------------------------//
			// Posiciona pedido de Vendas - DS2U - T.MORAES                            |
			//-------------------------------------------------------------------------//
			dbSelectArea("SD2")
			dbSetOrder(3)
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				//-------------------------------------------------------------------------//
				// Verifica campo customizado pedido de venda (SC5) - DS2U - T.MORAES      |
				//-------------------------------------------------------------------------//
				If lSC5 .AND. !Empty(SC5->C5_MSEMAIL)
					aadd(aDest,SC5->C5_MSEMAIL) //16 - Email informado no pedido de vendas
				Else
